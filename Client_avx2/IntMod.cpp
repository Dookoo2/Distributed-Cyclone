...
{
    // Временные переменные именно "unsigned long long", чтобы точно совпадать со сигнатурой _mulx_u64
    unsigned long long hi64 = 0;
    unsigned long long lo64 = _mulx_u64((unsigned long long)x, 
                                        (unsigned long long)y, 
                                        &hi64);
    // Записываем старшие 64 бита в *high, а возвращаем младшие
    *high = (uint64_t)hi64;
    return (uint64_t)lo64;
}

// Теперь подменяем ваш _umul128 на эту функцию:
#  undef _umul128
...
{
    // Убедимся, что p > 2:
    // (можно добавить проверку "p is prime", но здесь предполагаем, что p уже простое)
    // -------------------------------------------------------------------------
    // Сначала приводим a к остатку по p:
    Int A(a);
    A.Mod(&p);
    if (A.IsZero()) {
        // a ≡ 0 (mod p) => символ Лежандра = 0
        return 0;
    }

    int result = 1;  // будет хранить +1 или -1 (знак символа)

    // Будем повторять цикл, пока A != 0
    // В конце P станет равен gcd(старых A,p).
    // Если gcd=1 => результат = ±1.
    // Если gcd≠1 => результат = 0 (но для простого p это значит либо A=0, либо a делилось на p).
    Int P(p);

    while (!A.IsZero()) {
        // 1) Выносим факторы 2 из A ( A = 2^k * A', где A' нечетно )
        while (A.IsEven()) {
            A.ShiftR(1);  // делим A на 2
            // Умножаем результат на (2/p)
            //   (2/p) = (-1)^((p^2 - 1)/8) = -1, если p ≡ 3 или 5 (mod 8), иначе +1
        ...
        // 2) Теперь и A, и P — нечетные. Применяем закон квадратичной взаимности:
        //    (A|P)*(P|A) = (-1)^{(A-1)(P-1)/4}
        //    => если A ≡ 3 (mod 4) и P ≡ 3 (mod 4), то доп. множитель -1
    ...
    // Выйдя из цикла, если P == 1 => gcd=1 => возвращаем накопленный знак
    // Иначе (P != 1) => значит исходное A имело нетривиальный общий делитель с p,
    // но p — простое, так что это возможно только если A ≡ 0 mod p, мы это обработали раньше => 0.
    return P.IsOne() ? result : 0;
}
// ------------------------------------------------
bool Int::HasSqrt()
{
    // Предположим, что _P — это член класса, в котором хранится ваш простой модуль p
    int ls = LegendreSymbol(*this, _P);
    // Старый код (через a^((p-1)/2)) возвращал true ТОЛЬКО если результат == 1.
    // Т.е. 0 и -1 считались "false".
    // Чтобы сохранить точно ту же семантику, делаем:
    return (ls == 1);
}
